...
def day4part1(puzzleList):
    fields = ["byr:","iyr:","eyr:","hgt:","hcl:","ecl:","pid:"]
    validCount = 0
    for passport in puzzleList :
        valid = 1
        for field in fields :
...
def getField(passport : str,field : str) -> str:
    if field in passport :
        start = passport.find(field)+4
        end = passport.find("\n",start)
        if end==-1 or -1<passport.find(" ",start)<end :
            end = passport.find(" ",start)
        if end == -1 :
            end = len(passport)
...
def day4part2(puzzleList):
    fields = ["byr:","iyr:","eyr:","hgt:","hcl:","ecl:","pid:"]
    validCount = 0
    for passport in puzzleList :
        valid = 1
        try:
            field = getField(passport,"byr:")
            if not(len(field) == 4 and 1920 <= int(field) <= 2002) :
                valid = 0
                print("Invalid field : " + "byr------->" + field)

            field = getField(passport, "iyr:")
            if not(len(field) == 4 and 2010 <= int(field) <= 2020) :
                valid = 0
                print("Invalid field : " + "iyr------->" + field)

            field = getField(passport, "eyr:")
            if not (len(field) == 4 and 2020 <= int(field) <= 2030):
                valid = 0
                print("Invalid field : " + "eyr------->" + field)

            field = getField(passport, "hgt:")
            if not (len(field) == 5 and 150 <= int(field[0:3]) <= 193 and field[3:5]=="cm") and not (len(field) == 4 and 59 <= int(field[0:2]) <= 76 and field[2:4]=="in"):
                valid = 0
                print("Invalid field : " + "hgt------->" + field)

            field = getField(passport, "hcl:")
            if not (len(field) == 7 and field[0]=="#"):
                valid = 0
                print("Invalid field : " + "hcl------->" + field)
            else :
                for i in field[1:7] :
            ...
            field = getField(passport, "ecl:")
            if not(field in ["amb","blu","brn","gry","grn","hzl","oth"]) :
                valid = 0
                print("Invalid field : " + "ecl------->" + field)

            field = getField(passport, "pid:")
            if not (len(field)==9):
                valid = 0
                print("Invalid field : " + "pid------->" + field)
            else :
                for i in field :
...
print(day4part1(puzzleList))
print(day4part2(puzzleList))
