def processPuzzleInput(puzzleInput : str) -> list :
    return [i.split(":") for i in puzzleInput.split("\n")]


def day2part1(puzzleList) :
    validated = 0
    for line in puzzleList :
        key = line[0][-1]
        [keymin, keymax] = [int(i) for i in line[0].split(" ")[0].split("-")]
        if line[1].count(key)>=keymin and line[1].count(key)<=keymax :
            validated+=1
    return validated

def day2part2(puzzleList) :
    validated = 0
    for line in puzzleList :
        key = line[0][-1]
        [keymin, keymax] = [int(i) for i in line[0].split(" ")[0].split("-")]
        if bool(line[1][keymin]==key) != bool(line[1][keymax]==key) :
            validated+=1
    return validated

puzzleInput = """13-14 f: ffffffffnfffvv
...
print(day2part1(puzzleList))
print(day2part2(puzzleList))